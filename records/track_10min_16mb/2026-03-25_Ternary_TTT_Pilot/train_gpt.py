...
# --- Ternary Quantization & Base-3 Packing ---

def quantize_ternary(w: Tensor, group_size: int = 64) -> Tensor:
    """BitNet b1.58 ternary quantization with STE. w: any shape."""
    orig_shape = w.shape
    w_bf16 = w.bfloat16()
    w_g = w_bf16.reshape(-1, group_size)
    scale = w_g.abs().mean(-1, keepdim=True).clamp(min=1e-8)
    q = (w_g / scale).round().clamp(-1, 1)
    w_ternary = w_bf16 + ((q * scale).reshape(orig_shape) - w_bf16).detach()
    return w_ternary

def pack_ternary(q: Tensor) -> tuple[bytes, int]:
    """Pack ternary weights (-1, 0, 1) into bits using base-3 (5 trits/byte)."""
    ...
    # Using the 74M script's base-3 packing formula
    packed = (g[:, 0] + g[:, 1] * 3 + g[:, 2] * 9 + g[:, 3] * 27 + g[:, 4] * 81).tobytes()
    return packed, n

def unpack_ternary(data: bytes, n: int) -> Tensor:
    """Unpack base-3 encoded ternary weights."""
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        if self.ternary_enabled:
            qo_bank = quantize_ternary(self.qo_bank, self.bitnet_group_size)
            kv_bank = quantize_ternary(self.kv_bank, self.bitnet_group_size)
            mlp_up_bank = quantize_ternary(self.mlp_up_bank, self.bitnet_group_size)
            mlp_down_bank = quantize_ternary(self.mlp_down_bank, self.bitnet_group_size)
        else:
            qo_bank, kv_bank = self.qo_bank, self.kv_bank
            mlp_up_bank, mlp_down_bank = self.mlp_up_bank, self.mlp_down_bank
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                qo_bank[i], kv_bank[i], kv_bank[n + i],
                qo_bank[n + i], mlp_up_bank[i], mlp_down_bank[i],
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                qo_bank[bi], kv_bank[bi], kv_bank[n + bi],
                qo_bank[n + bi], mlp_up_bank[bi], mlp_down_bank[bi],
                v_embed=ve, v0=v0)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], ternary_enabled: bool = False):
    ...
    for name, tensor in state_dict.items():
        ...
        if ternary_enabled and cat in {"mlp", "attn"}:
            w_g = t.reshape(-1, 64)
            scale = w_g.abs().mean(-1, keepdim=True).clamp(min=1e-8)
            q = (w_g / scale).round().clamp(-1, 1)
            packed, n = pack_ternary(q)
            result[name + ".packed"] = torch.from_numpy(np.frombuffer(packed, dtype=np.uint8))
            result[name + ".scale"] = scale.to(torch.float16)
            meta[name] = {"type": "ternary", "n": n, "shape": t.shape}
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if info in ("passthrough", "passthrough_ctrl"):
            t = result[name]
            out[name] = t.to(orig_dtype) if t.is_floating_point() else t
            continue
            
        if isinstance(info, dict) and info.get("type") == "ternary":
            packed_bytes = result[name + ".packed"].numpy().tobytes()
            n, shape = info["n"], info["shape"]
            q = unpack_ternary(packed_bytes, n).reshape(shape)
            s = result[name + ".scale"]
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        ternary_enabled=args.ternary_enabled,
    ...
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, ternary_enabled=args.ternary_enabled)
    ...
    eval_model = GPT(
        ...
        ternary_enabled=args.ternary_enabled, bitnet_group_size=args.bitnet_group_size,
    ).to(device).bfloat16()
